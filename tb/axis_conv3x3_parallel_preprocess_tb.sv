...
// Module: axis_conv3x3_parallel_preprocess_tb
// Description:
//Verifies the AXI4-Stream learned INT8 3x3 parallel convolution wrapper.
...
module axis_conv3x3_parallel_preprocess_tb #(
    ...
    initial begin : provide_stimulus
        ...
        $timeformat(-9, 0, " ns");
        $display("AXI4-Stream Parallel Conv input MEM:    %s", input_mem_path);
        $display("AXI4-Stream Parallel Conv expected MEM: %s", expected_mem_path);
        ...
        if (mismatch_count == 0) begin
            $display(
                "PASS: AXI4-Stream Parallel Conv matched %0d pixels in %0d cycles.",
                IMAGE_PIXELS,
                processing_cycles
            );
            $finish;
        ...
        $fatal(1, "FAIL: AXI4-Stream Parallel Conv test found %0d issue(s).", mismatch_count);
    end
            ...
            if (aresetn === 1'b0) begin
                ...
                if (output_count >= IMAGE_PIXELS) begin
                    ...
                    if (m_axis_tdata[PIXEL_WIDTH-1:0] !== expected_pixels[output_count]) begin
                        mismatch_count++;
                        $error(
                            "Parallel convolution pixel mismatch at output %0d: actual=0x%02h expected=0x%02h",
                            output_count,
            ...
            if (aresetn === 1'b0) begin
                ...
                if (was_stalled && (m_axis_tvalid === 1'b1) && (m_axis_tready === 1'b0)) begin
                    if ((m_axis_tdata !== held_tdata) ||
                        ...
                        mismatch_count++;
                        $error("AXI4-Stream Parallel Conv output changed while stalled.");
                    end
                end
