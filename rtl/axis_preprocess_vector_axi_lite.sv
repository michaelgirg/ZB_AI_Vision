...
module axis_preprocess_vector_axi_lite #(
    ...
    assign vector_commit_pulse =
        reg_write_en && (reg_write_addr == ADDR_VECTOR_CFG_COMMIT) &&
        ...
        if (rst) begin
            ...
            if (reg_write_en && !stream_busy) begin
                unique case (reg_write_addr)
                    ...
                    ADDR_VECTOR_CFG_INDEX : begin
                        vector_cfg_index_r <= reg_write_data[5:0];
                    end
                    ...
                    ADDR_VECTOR_CFG_DATA : begin
                        if (vector_cfg_index_r[3:0] < VECTOR_TAPS) begin
                            ...
                            vector_shadow_bias_r[vector_cfg_index_r[5:4]] <= reg_write_data;
                        end else if (vector_cfg_index_r[3:0] == 4'd10) begin
                            vector_shadow_shift_r[vector_cfg_index_r[5:4]] <= reg_write_data[4:0];
                            vector_shadow_relu_r[vector_cfg_index_r[5:4]] <= reg_write_data[8];
                        end
                    end
